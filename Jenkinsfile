pipeline{
    ...
    stages {
        ...
        stage('deploy'){
		    ...
		    script{
                        images = sh(returnStdout: true, script: 'sshpass -p ${PASSWORD} ssh ${SSH_CRED} docker images -aq').replaceAll("\n", " ")
                    }
                    sh ("sshpass -p ${PASSWORD} ssh ${SSH_CRED}  if ! [ -z $images ]; then docker rmi $images; fi")                    
